            ...
            <n-gi :span="3">
                <n-card title="消息">
                    ...
                    <n-alert v-if="isTermExpiringSoon" title="您的会员即将到期" type="info" style="margin-bottom: 10px">
                        您的LingYu Byte AI{{ userInfo?.usergroup }}将于 {{ userInfo?.term }} 到期，剩余 {{ remainingDays }} 天，请及时续费。
                    </n-alert>
                </n-card>
...
const copyAndToggleContent = () => {
    navigator.clipboard.writeText(userInfo?.usertoken || "LingYu Byte AI账户Token复制失败").then(() => {
