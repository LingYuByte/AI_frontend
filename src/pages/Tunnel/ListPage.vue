                    ...
                    <n-descriptions label-placement="left" size="medium" :column="screenWidth >= 600 ? 3 : 1" bordered>
                        ...
                        <n-descriptions-item label="带宽">
                            ...
                            <n-tooltip v-else-if="NodeInfo.china === 'no'" trigger="hover">
                                ...
                                此节点走LingYu Byte AI国外带宽，您的国外带宽上限为{{ userInfo?.bandwidth ? userInfo.bandwidth * 4 : 32 }}m
                            </n-tooltip>
                            <n-tooltip v-else trigger="hover">
                                <template #trigger>
                                    国内带宽
                                </template>
                                此节点走LingYu Byte AI国内带宽，您的国内带宽上限为{{ userInfo?.bandwidth }}m
                            </n-tooltip>
                        </n-descriptions-item>
            ...
            if (data.state === 'success') {
                try {
                    ...
                    if (data.state === 'success') {
                        ...
                        dialog.success({
                            title: '成功',
                            content: '隧道创建成功！但是您使用了LingYu Byte AI提供的免费域名，域名解析通常不会立即生效，一般在48小时内彻底生效，部分DNS会在几分钟内生效。简而言之，您创建的免费域名需要等待一段时间后才能正常使用。',
                            positiveText: '我知道了',
                            onPositiveClick: () => {
